...
const Private = ({ account, networkName }) => {
    ...
    const loadOwnedItems = async (address) => {
        ...
        const response = await fetch(`http://127.0.0.1:5000/api/create/fetchprivatenfts`, {
            method: 'POST',
            headers: {
    ...
    return (
        <div className='container my-5'>
            <ToastContainer
                ...
                theme="light"
            />
            <h1 className='my-5' style={{ textAlign: "center" }}>Private Items</h1>
            {allItems.length > 0 ? (
                <div className="row">
                        ...
                        <div key={idx} className="col my-3" style={{ maxWidth: "fit-content" }}>
                            <div className="overflow-hidden" style={{ width: "16rem", height: "22.5rem", boxShadow: "15px 15px 6px silver", border: "1px dashed black", borderRadius: "10%" }}>
                                ...
                                <div className="card-body">
                                    ...
                                    <div className='my-2 mx-3'>Item is stacking with <b>{item.price} </b>ETH</div>
                                    <button className="btn btn-primary my-1 mx-3" style={{ textAlign: "center", width: "14rem", borderRadius: "30px" }} onClick={() => addCart(item)}>Add to cart</button>
                                </div>
                            </div>
                        </div>
                    ))}
